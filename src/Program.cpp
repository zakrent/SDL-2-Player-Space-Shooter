...
Program::Program() : controller1(0), controller2(1){
	bodies.push_back(new Planet(Vector2D(mainRenderer.WINDOW_WIDTH/2,mainRenderer.WINDOW_HEIGHT/2),Vector2D(0,0),5,1800,&bodies));
	bodies.back()->isStatic = true;
	bodies.push_back(new Planet(Vector2D((mainRenderer.WINDOW_WIDTH/2)-300,mainRenderer.WINDOW_HEIGHT/2),Vector2D(0,-0.775),2,50,&bodies));
	bodies.push_back(new Planet(Vector2D((mainRenderer.WINDOW_WIDTH/2)-100,mainRenderer.WINDOW_HEIGHT/2),Vector2D(0,1.34),3,100,&bodies));
...
void Program::startMainLoop(){
	...
	while(running){
		...
		if(controller1.controlerPlayer == NULL){
			Player* tempP1 = new Player(Vector2D((mainRenderer.WINDOW_WIDTH/2)-200,mainRenderer.WINDOW_HEIGHT/2),Vector2D(0,0.95),2,0.0001,&bodies, true);
			controller1.controlerPlayer = tempP1;
			bodies.push_back(tempP1);
		}
		if(controller2.controlerPlayer == NULL){
			Player* tempP2 = new Player(Vector2D((mainRenderer.WINDOW_WIDTH/2)+200,mainRenderer.WINDOW_HEIGHT/2),Vector2D(0,-0.95),2,0.0001,&bodies, false);
